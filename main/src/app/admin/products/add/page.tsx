...
export default function AddProductPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch categories
        const categoriesRes = await fetch(`${apiUrl}/category/all`);
        ...
        // Fetch brands
        const brandsRes = await fetch(`${apiUrl}/brand/all`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Submitting product with category ID:", formData.category);
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag, index) => (
              <div
                key={index}
                className="flex items-center gap-2 bg-blue-100 text-blue-700 px-3 py-1.5 rounded-full"
              >
